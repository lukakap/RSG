...
void recursiveAssemble(Production prod, map<string, Definition> grammar, string& toAssamble)
{
  for (Production::iterator curr = prod.begin(); curr != prod.end(); ++curr){
    ...
    if(str.rfind('<', 0) != 0)
    {
      //doesnt start with < , so it is terminal word
      toAssamble += str + " ";
    } else {
      recursiveAssemble(grammar[str].getRandomProduction(), grammar, toAssamble);
    }
  }

}

void generateVersions(map<string, Definition> grammar){
    ...
    for(int i = 0; i < numberOfVersions; i++)
    {
      ...
      //starter definition has oonly one production
      Production startProd = grammar[startNonTerminal].getRandomProduction();

      string toAssamble = "";  
      recursiveAssemble(startProd, grammar, toAssamble);  

      cout << toAssamble << endl;
    }
